...
#        flag = 0
  ...
  '''
  return (float(float(sum(gctime_list))/float(len(gctime_list))), float(float(sum(res_run))/float(len(res_run))), float(float(sum(gctime_list))/float(len(gctime_list)))/float(float(sum(res_run))/float(len(res_run))))

def eachplot(name, inlist1, inlist2):
  ''' plot each benchmark
  group by different gc threads, inlist1 is gclist, inlist2 is mutatorlist
  '''
  xlist = ['1\nParallel GC Thread: 1', '2', '4','8', '16', '32', '48', ' ','1\n    2', '2', '4','8', '16', '32', '48', ' ', '1\n   4', '2', '4','8', '16', '32', '48', ' ', '1\n   8', '2', '4','8', '16', '32', '48']
  ylist1 = [] # gc fractions
...
#  plt.bar(range(len(ylist2)), ylist2, bottom=ylist1, color='#d3d3d3', label="Completion time")
  ...
  plt.savefig(name+"_different_GCThread.pdf", format='pdf', dpi=900, bbox_inches='tight')
  plt.cla()
...
#  namelist = ['lusearch', 'xalan', 'sunflow', 'pmd', 'eclipse', 'jython', 'scalac', 'scaladoc', 'scalatest']
...
#    print list1, list2
  ...
  plt.bar(range(len(ylist1)), ylist1, color="#000000", label="Pause Time")
  plt.bar(range(len(xlist)), ylist2, bottom=ylist1, color='#d3d3d3', label="Completion time")
  plt.xticks(range(len(xlist)), xlist)
#  plt.legend(loc="upper center", ncol=2)
#  plt.gca()
  plt.grid()
  ...
  plt.savefig("overall_performance.pdf", format='pdf', dpi=900, bbox_inches='tight')
  plt.cla()
  '''
...
def main():
  ...
  if os.path.isfile(filename):
    ...
    for root, dirs, files in os.walk(filename):
      ...
      fnames = sorted(fnames, key=lambda x: int(x.split('_')[1], 0))
      for f in fnames:
        print f, helper(f)
'''
