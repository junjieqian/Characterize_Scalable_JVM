	...
	if [ $thread -eq 1 ]; then
		...
		do
			...
			do
			    ...
			    set -x
				java -Xms40g -Xmx40g -verbose:gc -XX:+PrintGCApplicationConcurrentTime -XX:+PrintGCApplicationStoppedTime -XX:+UseParallelGC -jar $Dacapo $name >> $LOGFILE"$name"_"$thread"
#	            sleep 20
			...
			do
				...
				set -x
				java -Xms40g -Xmx40g -verbose:gc -XX:+PrintGCApplicationConcurrentTime -XX:+PrintGCApplicationStoppedTime -XX:+UseParallelGC -jar $Scala $sname >> $LOGFILE"$sname"_"$thread"
				sleep 20
...
do
   if [ $thread -eq 1 ]; then
      ...
      python set_cpus.py 0-47
	   ...
	   for i in {1..10}; do
		   java -Xms40g -Xmx40g -verbose:gc -XX:+PrintGCApplicationConcurrentTime -XX:+PrintGCApplicationStoppedTime -XX:+UseParallelGC -jar $Dacapo "lusearch" >> $LOGFILE"lusearch"_"$thread"
