...
dacapobenchmark=(["eclipse"]="480m" ["jython"]="180m" ["sunflow"]="72m" ["xalan"]="180m" ["h2"]="600m")
	...
	if [ $thread -eq 1 ]; then
		...
		do
			...
			do
			    ...
			    set -x
				java -Xms${dacapobenchmark["$name"]} -Xmx${dacapobenchmark["$name"]} -verbose:gc -XX:+PrintGCDetails -XX:+PrintGCApplicationConcurrentTime -XX:+PrintGCApplicationStoppedTime -XX:+UseParallelGC -jar $Dacapo $name >> $LOGFILE"$name"_"$thread"
	            sleep 20
			...
			do
				...
				set -x
				java -Xms${scalabenchmark["$sname"]} -Xmx${scalabenchmark["$sname"]} -verbose:gc -XX:+PrintGCDetails -XX:+PrintGCApplicationConcurrentTime -XX:+PrintGCApplicationStoppedTime -XX:+UseParallelGC -jar $Scala $sname >> $LOGFILE"$sname"_"$thread"
				sleep 20
...
do
   if [ $thread -eq 1 ]; then
      ...
      python set_cpus.py 0-47
	   ...
	   for i in {1..10}; do
		   java -Xms96m -Xmx96m -verbose:gc -XX:+PrintGCDetails -XX:+PrintGCApplicationConcurrentTime -XX:+PrintGCApplicationStoppedTime -XX:+UseParallelGC -jar $Dacapo "lusearch" >> $LOGFILE"lusearch"_"$thread"
