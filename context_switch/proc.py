...
#      t1 = int(round(time.time() * 1000))
      ...
      for thread in threads:
        try:
          ...
          continue
        if not thread in rundict:
          rundict[thread] = d["state"]
          runcountdict[thread] = 0
        if (d["state"] == "R" or rundict[thread] == "R") and (not d["state"] == rundict[thread]):
            runcountdict[thread] += 1
