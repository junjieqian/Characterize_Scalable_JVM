...
for thread in 1 2 4 8 16 32 48; do
	...
	for name in "${!dacapobenchmark[@]}"; do
		python proc.py stdin stdout $name java -Xmx${dacapobenchmark["$name"]} -Xms${dacapobenchmark["$name"]} -XX:+UseParallelGC -jar $Dacapo $name >> "results/""$name"
	done

	for sname in ${!scalabenchmark[@]}; do
		python proc.py stdin stdout $sname java -Xmx${scalabenchmark["$sname"]} -Xms${scalabenchmark["$sname"]} -XX:+UseParallelGC -jar $Scala $sname >> "results/"$sname
	done
done
