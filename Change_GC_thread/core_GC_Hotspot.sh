	...
	do
		...
		do
			...
			do
			    ...
			    set -x
				java -Xms${dacapobenchmark["$name"]} -Xmx${dacapobenchmark["$name"]} -verbose:gc -XX:+PrintGCApplicationConcurrentTime -XX:+PrintGCApplicationStoppedTime -XX:+UseParallelGC -XX:ParallelGCThreads=$gcthread -jar $Dacapo $name >> $LOGFILE"$name"_"$thread"_$gcthread
#	            sleep 20
			...
			do
				...
				set -x
				java -Xms${scalabenchmark["$sname"]} -Xmx${scalabenchmark["$sname"]} -verbose:gc -XX:+PrintGCApplicationConcurrentTime -XX:+PrintGCApplicationStoppedTime -XX:+UseParallelGC -XX:ParallelGCThreads=$gcthread -jar $Scala $sname >> $LOGFILE"$sname"_"$thread"_$gcthread
				sleep 20
...
do
   ...
   do
	   for i in {1..4}; do
		   java -Xms96m -Xmx96m -verbose:gc -XX:+PrintGCApplicationConcurrentTime -XX:+PrintGCApplicationStoppedTime -XX:+UseParallelGC -XX:ParallelGCThreads=$gcthread -jar $Dacapo "lusearch" >> $LOGFILE"Lusearch"_"$thread"_$gcthread
